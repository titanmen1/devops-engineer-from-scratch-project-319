#!/usr/bin/env bash
#
# Готовит хранилище состояния Terraform: сервисный аккаунт, статический ключ
# доступа и бакет с версионированием. Запускается один раз перед первым
# `terraform init`, потому что сам бакет описать в Terraform нельзя — состояние
# должно куда-то лечь ещё до первого apply.
#
# Скрипт идемпотентный: повторный запуск ничего не ломает, но новый ключ не
# создаёт. Если ключ потерян, удалите старый (`yc iam access-key list`) и
...
# Версии состояния спасают, если apply испортил state: старую версию всегда
# можно достать из бакета.
